...
--- Apply user settings.
---@param values table
function test.setup(values) setmetatable(config, { __index = vim.tbl_extend('force', config.defaults, values) }) end

--- Set the colorscheme.
function test.colorscheme()
  if vim.version().minor < 8 then
    vim.notify('Neovim 0.8+ is required for this custom theme', vim.log.levels.ERROR, { title = 'Custom theme' })
    return
  end
